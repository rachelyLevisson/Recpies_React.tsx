...
export default function Home() {
  ...
  const handleUpdateRecipe = async (updatedRecipe: Recipe) => {
    try {
      console.log("Updating recipe:", JSON.stringify(updatedRecipe, null, 2))

      // Step 1: Delete the existing recipe
      await axios.post(`http://localhost:8080/api/recipe/delete/${updatedRecipe.Id}`)

      // Step 2: Create a new recipe with the same ID and updated data
      const recipeToCreate = {
        Id: updatedRecipe.Id, // Keep the same ID
        ...
        Ingridents: updatedRecipe.Ingridents,
        Instructions: updatedRecipe.Instructions,
      }

      // Create a new recipe with the same data
      await axios.post("http://localhost:8080/api/recipe", recipeToCreate, {
        headers: {
          "Content-Type": "application/json",
        },
      })
      ...
      // If there's an error, refresh recipes to ensure UI is in sync with server
      fetchRecipes()
    }
  }
